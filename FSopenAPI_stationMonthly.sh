...
# retrieve monthlyKPI, assuming only one Plant
...
returnhist=$(curl -s -X POST  "${monthlyURL}" \
     -H "Content-Type: application/json" \
     -H "XSRF-Token: ${token}" \
     -d "{\"stationCodes\": \"${plant_code}\",\"collectTime\": \"${current_hour_timestamp_ms}\"}")

log_verbose "monthly KPI Station output:"
echo "${returnhist}"
echo ""
...
if [[ "$writetodatabase" == "true" ]]; then
	...
	query="INSERT INTO ${mysql_table_prefix}kpimonthly (collectTime, stationCode, radiation_intensity, power_profit, theory_power, ongrid_power, inverter_power) VALUES "
